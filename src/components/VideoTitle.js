...
const VideoTitle = ({ title, overview, onPlay, showTrailer }) => {
  ...
  return (
    <div className='w-screen aspect-video absolute text-white bg-gradient-to-r h-60 md:h-screen from-black'>
      <div className="absolute left-[-40px] lg:left-0 top-18  md:top-2/5 lg:top-1/2 transform  -translate-y-1/24 md:-translate-y-3/5 w-[150px]  md:w-xs lg:w-sm 2xl:w-3xl space-y-2 lg:space-y-4 2xl:space-y-10 ml-15 md:ml-20">
        <h1 className='text-[10px] md:text-3xl lg:text-5xl 2xl:text-8xl font-bold lg:mb-8'>{title}</h1>
